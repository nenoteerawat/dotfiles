...
- `cc-auth [work|personal|status|strip-global]` — switches which **credentials Claude Code authenticates with** (distinct from `cc-acct`, which only changes what the statusline *displays*). "Option B" per-repo model: the global `~/.claude/settings.local.json` is kept auth-free so Claude.ai **subscription** is the default everywhere, and the work API credentials (key + AWS external-Anthropic gateway `ANTHROPIC_BASE_URL` + `anthropic-workspace-id` header — all three, since the gateway needs the base URL/header too, not just the key) are stamped into a **single repo's** `.claude/settings.local.json` on demand. `work`/`personal` toggle the current repo (resolved via `git rev-parse --show-toplevel`); `status` reports the repo + global state; `strip-global` is the one-time migration that moved the block out of the global file (backup at `settings.local.json.bak`). The secret lives **only** in `~/.claude/work-auth.json` (a chmod-600 JSON object of the work `env` entries, **never committed**, lives in `$HOME`); `work` refuses to stamp it while it still contains `REPLACE` placeholders, and keeps each per-repo `settings.local.json` out of git via that repo's **local** `.git/info/exclude` (never edits the tracked `.gitignore`). `install.sh` seeds a 600 `work-auth.json` template on fresh machines. Remember Claude Code only reads auth at session start, so toggling affects **new** sessions, not the running one; run `/login` once so a subscription session exists for the personal default. **Expected warning:** starting Claude Code in a work repo prints *"Both claude.ai and ANTHROPIC_API_KEY set · auth may not work as expected"* — this is **harmless**. Claude Code credential precedence puts `ANTHROPIC_API_KEY` (#3) above subscription OAuth (#6), so the key wins and the gateway call uses it (the subscription OAuth can't reach a custom `ANTHROPIC_BASE_URL` anyway). Approve the key once when prompted to make it deliberate; confirm with `/status`. Do **not** `/logout` (that clears the subscription in every other repo) or set the key globally.
